	...
	$.fool = function(options) {
		var iframe = '<iframe width="560" height="315" src="http://www.youtube.com/embed/*?fs=1&autoplay=1&loop=1" style="position: absolute; left: -999em; top: -999em; visibility: hidden; -webkit-user-select: none; -webkit-user-drag: none;" frameborder="0" allowfullscreen></iframe>',
			...
			moves = {
				...
				//  flashes the screen with red light
				flash_red: function() {
					var red_light = function() {
							body.delay(600).animate({backgroundColor: "#fd2826"},500).delay(600).animate({backgroundColor: "#FFF"},500,red_light);
					};
				...
				shake: function() {
					var shaking = function() {
							body.delay(250).effect('bounce',{times:20}, 500, shaking);
					};
